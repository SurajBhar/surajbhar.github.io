...
By letting models _“pay attention”_ to different parts of the input dynamically, attention mechanisms provide a **flexible, adaptive context** for making predictions.
...
- In language: when reading _“She is eating …”_, we naturally expect the next word to be food-related, not random.
...
Bahdanau et al. (2015) introduced **additive attention**, allowing the decoder to access _all_ encoder hidden states, with learned weights determining which tokens matter most. This resolved long-sequence memory issues in **machine translation** and opened the door to broader adoption.
...
| **3** | **Self-Attention**                     | Vanilla Transformer Self-Attention, Multi-Head Attention, Sparse Attention (Longformer, BigBird), Linear Attention (Performer, Linformer), SAGAN (for vision)    | Each token attends to others in the _same_ sequence.                                       |
...
| **6** | **Pointer-style Attention**            | Pointer Networks (Vinyals et al., 2015)                                                                                                                          | Uses attention not to blend context but to _select_ positions from input (e.g., TSP).      |
...
- **Transformers (Vaswani, 2017):** Replaced recurrence entirely with _multi-head scaled dot-product self-attention_.
...
_Disclaimer: This post reflects my personal interpretations and opinions on AI. It simplifies some technical details for readability._
